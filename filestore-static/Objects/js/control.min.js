...
frappe.ui.form.ControlTextEditor = frappe.ui.form.ControlCode.extend({
	...
	set_in_editor: function set_in_editor(value) {
		...
		if (!this._last_change_on || time_since_last_keystroke > 3000) {
			setTimeout(function () {
				return _this.setting_count = 0;
			}, 500);
			this.editor.summernote('code', value || '');
		} else {
			this._setting_value = setInterval(function () {
				if (time_since_last_keystroke > 3000) {
					if (_this.last_value !== _this.get_input_value()) {
						_this.editor.summernote('code', _this.last_value || '');
					}
					clearInterval(_this._setting_value);
					_this._setting_value = null;
					_this.setting_count = 0;
				}
			}, 1000);
		}
	},
	...
	setup_image_dialog: function setup_image_dialog() {
		...
		this.note_editor.find('[data-original-title="Image"]').on('click', function () {
			if (!_this2.image_dialog) {
				_this2.image_dialog = new frappe.ui.Dialog({
					title: __("Image"),
			...
			_this2.image_dialog.show();
			_this2.image_dialog.get_field("upload_area").$wrapper.empty();

			var attachments = _this2.frm && _this2.frm.attachments.get_attachments() || [];
			var select = _this2.image_dialog.get_field("select");
			if (attachments.length) {
				...
				select.toggle(true);
				_this2.image_dialog.get_field("or_attach").toggle(true);
				select.refresh();
			} else {
				_this2.image_dialog.get_field("or_attach").toggle(false);
				select.toggle(false);
			}
			select.$input.val("");

			_this2.set_upload_options();
			frappe.upload.make(_this2.upload_options);
		});
	}
});
