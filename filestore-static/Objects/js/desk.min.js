...
frappe.chart.FrappeChart = function () {
		...
		value: function set_width() {
			...
			this.specific_values.map(function (val) {
				if (_this2.get_strwidth(val.title) > special_values_width) {
					special_values_width = _this2.get_strwidth(val.title);
				}
			});
		...
		value: function setup_utils() {
			...
			this.animate = {
				'bar': function bar(_bar, new_y, args) {
					return [_bar, { height: args.new_height, y: new_y }, 300, "easein"];
				},
				'dot': function dot(_dot, new_y) {
					return [_dot, { cy: new_y }, 300, "easein"];
				}
			};
		}
	}]);
...
frappe.chart.AxisChart = function (_frappe$chart$FrappeC) {
	_inherits(AxisChart, _frappe$chart$FrappeC);

	function AxisChart(args) {
		_classCallCheck(this, AxisChart);

		var _this6 = _possibleConstructorReturn(this, (AxisChart.__proto__ || Object.getPrototypeOf(AxisChart)).call(this, args));

		_this6.x = _this6.data.labels;
		_this6.y = _this6.data.datasets;

		_this6.get_x_label = _this6.format_lambdas.x_label;
		_this6.get_y_label = _this6.format_lambdas.y_label;
		_this6.get_x_tooltip = _this6.format_lambdas.x_tooltip;
		_this6.get_y_tooltip = _this6.format_lambdas.y_tooltip;

		_this6.colors = ['lightblue', 'purple', 'blue', 'green', 'lightgreen', 'yellow', 'orange', 'red'];
		return _this6;
	}
	...
	_createClass(AxisChart, [{
		...
		value: function setup_x() {
			...
			this.x_axis_values = this.x.map(function (d, i) {
				return frappe.chart.utils.float_2(_this7.x_offset + i * _this7.avg_unit_width);
			});
		}
	}, {
		key: "setup_y",
		...
		value: function setup_components() {
			...
			_get(AxisChart.prototype.__proto__ || Object.getPrototypeOf(AxisChart.prototype), "setup_components", this).call(this);
		}
	}, {
		...
		value: function make_y_axis() {
			...
			if (this.y_axis_group.textContent) {} else {}
			...
			this.y_axis_values.map(function (point) {
				...
				var y_level = $$.createSVG('g', {
					...
					transform: "translate(0, " + (_this8.height - point * _this8.multiplier) + ")"
				});

				y_level.appendChild(line);
				y_level.appendChild(text);

				_this8.y_axis_group.appendChild(y_level);
			});
		}
	}, {
		key: "make_x_axis",
		value: function make_x_axis() {
			...
			this.x.map(function (point, i) {
				var allowed_space = _this9.avg_unit_width * 1.5;
				if (_this9.get_strwidth(point) > allowed_space) {
				...
				var x_level = $$.createSVG('g', {
					className: "tick " + label_class,
					transform: "translate(" + _this9.x_axis_values[i] + ", 0)"
				});

				x_level.appendChild(line);
				x_level.appendChild(text);

				_this9.x_axis_group.appendChild(x_level);
			});
		}
	}, {
		key: "draw_graph",
		value: function draw_graph() {
			...
			this.y.map(function (d, i) {
				d.y_tops = new Array(d.values.length).fill(_this10.height);
				data.push({ values: d.values });
				d.svg_units = [];

				_this10.make_new_units_for_dataset(d.y_tops, d.color || _this10.colors[i], i);
				_this10.make_path && _this10.make_path(d, d.color || _this10.colors[i]);
			});

			setTimeout(function () {
				_this10.update_values(data);
			}, 500);
		}
	}, {
		key: "setup_navigation",
		value: function setup_navigation() {
			...
			setTimeout(function () {
				_get(AxisChart.prototype.__proto__ || Object.getPrototypeOf(AxisChart.prototype), "setup_navigation", _this11).call(_this11);
			}, 1000);
		}
	}, {
		key: "make_new_units_for_dataset",
		value: function make_new_units_for_dataset(y_values, color, dataset_index) {
			var _this12 = this;
			...
			y_values.map(function (y, i) {
				var data_unit = _this12.draw[d.type](_this12.x_axis_values[i], y, d.args, color, dataset_index);
				_this12.svg_units_group.appendChild(data_unit);
				_this12.y[dataset_index].svg_units.push(data_unit);
			});
		}
	}, {
		...
		value: function make_y_specifics() {
			...
			this.specific_values.map(function (d) {
				...
				var specific_y_level = $$.createSVG('g', {
					className: "tick",
					transform: "translate(0, " + (_this13.height - d.value * _this13.multiplier) + ")"
				});

				specific_y_level.appendChild(line);
				specific_y_level.appendChild(text);

				_this13.specific_y_lines.appendChild(specific_y_level);
			});
		}
	}, {
		key: "bind_tooltip",
		value: function bind_tooltip() {
			...
			this.chart_wrapper.addEventListener('mousemove', function (e) {
				var rect = _this14.chart_wrapper.getBoundingClientRect();
				var offset = {
					top: rect.top + document.body.scrollTop,
					left: rect.left + document.body.scrollLeft
				};
				var relX = e.pageX - offset.left - _this14.translate_x;
				var relY = e.pageY - offset.top - _this14.translate_y;

				if (relY < _this14.height + _this14.translate_y * 2) {
					_this14.map_tooltip_x_position_and_show(relX);
				} else {
					_this14.tip.hide_tip();
				}
			});
		}
	}, {
		...
		value: function map_tooltip_x_position_and_show(relX) {
			...
			for (var i = this.x_axis_values.length - 1; i >= 0; i--) {
				...
				if (relX > x_val - this.avg_unit_width / 2) {
					...
					var y = this.y_min_tops[i] + this.translate_y + 4;
					...
					var values = this.y.map(function (set, j) {
						return {
							...
							color: set.color || _this15.colors[j]
						};
					});
		...
		key: "update_values",
		value: function update_values(new_y) {
			var _this16 = this;
			...
			var old_upper_limit = this.upper_limit;
			this.setup_y();
			...
			if (this.y[0].path) {
				this.y.map(function (e, i) {
					...
					var args = [{ unit: _this16.y[i].path, object: _this16.y[i], key: 'path' }, { d: new_path_str }, 300, "easein"];
					elements_to_animate.push(args);
				});
			}

			var anim_svg = $$.runSVGAnimation(this.svg, elements_to_animate);
			this.chart_wrapper.innerHTML = '';
			this.chart_wrapper.appendChild(anim_svg);

			setTimeout(function () {
				_this16.chart_wrapper.innerHTML = '';
				_this16.chart_wrapper.appendChild(_this16.svg);
			}, 250);
		}
	}, {
		key: "update_y_axis",
		value: function update_y_axis() {
			var elements = [];
		...
		key: "update_x_axis",
		value: function update_x_axis() {}
	}, {
		key: "animate_for_equilength_data",
		value: function animate_for_equilength_data(elements_to_animate) {
			var _this17 = this;

			this.y.map(function (d) {
				d.y_tops = d.values.map(function (val) {
					return frappe.chart.utils.float_2(_this17.height - val * _this17.multiplier);
				});
				d.svg_units.map(function (unit, j) {
					elements_to_animate.push(_this17.animate[_this17.unit_args.type]({ unit: unit, array: d.svg_units, index: j }, d.y_tops[j], { new_height: _this17.height - d.y_tops[j] }));
				});
			});
			this.calc_min_tops();
		...
		key: "add_data_point",
		value: function add_data_point(data_point) {
		...
		key: "setup_metrics",
		value: function setup_metrics() {
			var values = this.get_all_y_values();

			var _get_upper_limit_and_ = this.get_upper_limit_and_parts(values);
		...
		key: "calc_all_y_tops",
		value: function calc_all_y_tops() {
			var _this18 = this;

			this.y.map(function (d) {
				d.y_tops = d.values.map(function (val) {
					return frappe.chart.utils.float_2(_this18.height - val * _this18.multiplier);
				});
			});
		}
	}, {
		...
		value: function calc_min_tops() {
			...
			this.y.map(function (d) {
				d.y_tops.map(function (y_top, i) {
					if (y_top < _this19.y_min_tops[i]) {
						_this19.y_min_tops[i] = y_top;
					}
				});
			});
		}
	...
	return AxisChart;
}(frappe.chart.FrappeChart);

frappe.chart.BarChart = function (_frappe$chart$AxisCha) {
	_inherits(BarChart, _frappe$chart$AxisCha);

	function BarChart() {
		_classCallCheck(this, BarChart);

		var _this20 = _possibleConstructorReturn(this, (BarChart.__proto__ || Object.getPrototypeOf(BarChart)).call(this, arguments[0]));

		_this20.type = 'bar-graph';
		_this20.setup();
		return _this20;
	}
	...
	_createClass(BarChart, [{
		key: "setup_values",
		value: function setup_values() {
			_get(BarChart.prototype.__proto__ || Object.getPrototypeOf(BarChart.prototype), "setup_values", this).call(this);
		...
		value: function bind_overlay() {
			...
			this.parent.addEventListener('data-select', function (e) {
				_this21.update_overlay(e.svg_unit);
			});
		}
	}, {
		key: "update_overlay",
		value: function update_overlay(unit) {
			...
			attributes.filter(function (attr) {
				...
				_this22.overlay.setAttribute(attr.name, attr.nodeValue);
			});
		}
	}, {
		key: "on_left_arrow",
	...
	return BarChart;
}(frappe.chart.AxisChart);

frappe.chart.LineChart = function (_frappe$chart$AxisCha2) {
	_inherits(LineChart, _frappe$chart$AxisCha2);

	function LineChart(args) {
		_classCallCheck(this, LineChart);

		var _this23 = _possibleConstructorReturn(this, (LineChart.__proto__ || Object.getPrototypeOf(LineChart)).call(this, args));

		if (Object.getPrototypeOf(_this23) !== frappe.chart.LineChart.prototype) {
			return _possibleConstructorReturn(_this23);
		}

		_this23.type = 'line-graph';
	...
	_createClass(LineChart, [{
		key: "setup_values",
		value: function setup_values() {
			_get(LineChart.prototype.__proto__ || Object.getPrototypeOf(LineChart.prototype), "setup_values", this).call(this);
			this.y_axis_mode = 'span';
		...
		value: function make_path(d, color) {
			...
			var points_list = d.y_tops.map(function (y, i) {
				return _this24.x_axis_values[i] + ',' + y;
			});
...
frappe.chart.RegionChart = function (_frappe$chart$LineCha) {
	_inherits(RegionChart, _frappe$chart$LineCha);

	function RegionChart(args) {
		_classCallCheck(this, RegionChart);

		var _this25 = _possibleConstructorReturn(this, (RegionChart.__proto__ || Object.getPrototypeOf(RegionChart)).call(this, args));

		_this25.type = 'region-graph';
		_this25.region_fill = 1;
		_this25.setup();
		return _this25;
	}

	return RegionChart;
}(frappe.chart.LineChart);

frappe.chart.PercentageChart = function (_frappe$chart$FrappeC2) {
	_inherits(PercentageChart, _frappe$chart$FrappeC2);

	function PercentageChart(args) {
		_classCallCheck(this, PercentageChart);

		var _this26 = _possibleConstructorReturn(this, (PercentageChart.__proto__ || Object.getPrototypeOf(PercentageChart)).call(this, args));

		_this26.x = _this26.data.labels;
		_this26.y = _this26.data.datasets;

		_this26.get_x_label = _this26.format_lambdas.x_label;
		_this26.get_y_label = _this26.format_lambdas.y_label;
		_this26.get_x_tooltip = _this26.format_lambdas.x_tooltip;
		_this26.get_y_tooltip = _this26.format_lambdas.y_tooltip;
	...
	_createClass(PercentageChart, [{
		...
		value: function setup_values() {
			...
			this.x.totals = this.x.map(function (d, i) {
				var total = 0;
				_this27.y.map(function (e) {
					total += e.values[i];
				});
				return total;
			});
		...
		value: function make_graph_components() {
			...
			this.x.totals.map(function (total, i) {
				var part = $$.create('div', {
					className: "progress-bar background " + _this28.x.colors[i],
					style: "width: " + total * 100 / _this28.grand_total + "%",
					inside: _this28.percentage_bar
				});
				_this28.x.units.push(part);
			});
		}
	}, {
		key: "bind_tooltip",
		value: function bind_tooltip() {
			...
			this.x.units.map(function (part, i) {
				part.addEventListener('mouseenter', function () {
					var g_off = _this29.chart_wrapper.offset(),
					    p_off = part.offset();

					var x = p_off.left - g_off.left + part.offsetWidth / 2;
					var y = p_off.top - g_off.top - 6;
					var title = (_this29.x.formatted && _this29.x.formatted.length > 0 ? _this29.x.formatted[i] : _this29.x[i]) + ': ';
					var percent = (_this29.x.totals[i] * 100 / _this29.grand_total).toFixed(1);

					_this29.tip.set_values(x, y, title, percent);
					_this29.tip.show_tip();
				});
			});
		}
	}, {
		...
		value: function show_summary() {
			...
			var x_values = this.x.formatted && this.x.formatted.length > 0 ? this.x.formatted : this.x;
			this.x.totals.map(function (d, i) {
				if (d) {
					var stats = $$.create('div', {
						className: 'stats',
						inside: _this30.stats_wrapper
					});
					stats.innerHTML = "<span class=\"indicator " + _this30.x.colors[i] + "\">\n\t\t\t\t\t<span class=\"text-muted\">" + x_values[i] + ":</span>\n\t\t\t\t\t" + d + "\n\t\t\t\t</span>";
				}
			});
		}
	}]);
...
frappe.chart.HeatMap = function (_frappe$chart$FrappeC3) {
	_inherits(HeatMap, _frappe$chart$FrappeC3);

	function HeatMap(_ref2) {
		var _ref2$start = _ref2.start,
		    start = _ref2$start === undefined ? new Date(moment().subtract(1, 'year').toDate()) : _ref2$start,
		...
		var _this31 = _possibleConstructorReturn(this, (HeatMap.__proto__ || Object.getPrototypeOf(HeatMap)).call(this, arguments[0]));

		_this31.type = 'heatmap';

		_this31.domain = domain;
		_this31.subdomain = subdomain;
		_this31.start = start;
		_this31.data = data;
		_this31.discrete_domains = discrete_domains;
		_this31.count_label = count_label;

		_this31.legend_colors = ['#ebedf0', '#c6e48b', '#7bc96f', '#239a3b', '#196127'];

		_this31.translate_x = 0;
		_this31.setup();
		return _this31;
	}
	...
	_createClass(HeatMap, [{
		...
		value: function render_month_labels() {
			...
			this.month_start_points.map(function (start, i) {
				var month_name = _this32.month_names[_this32.months[i]].substring(0, 3);

				$$.createSVG('text', {
					className: 'y-value-text',
					inside: _this32.domain_label_group,
					x: start + 12,
					y: 10,
					dy: '.32em',
		...
		value: function bind_tooltip() {
			...
			Array.prototype.slice.call(document.querySelectorAll(".data-group .day")).map(function (el) {
				el.addEventListener('mouseenter', function (e) {
					...
					var month = _this33.month_names[parseInt(date_parts[1]) - 1].substring(0, 3);

					var g_off = _this33.chart_wrapper.getBoundingClientRect(),
					...
					var value = count + ' ' + _this33.count_label;
					var name = ' on ' + month + ' ' + date_parts[0] + ', ' + date_parts[2];

					_this33.tip.set_values(x, y, name, value, [], 1);
					_this33.tip.show_tip();
				});
			});
		}
	}, {
...
frappe.chart.SvgTip = function () {
	function _class2(_ref3) {
...
frappe.chart.map_c3 = function (chart) {
	if (chart.data) {
		var data = chart.data;
		var type = chart.chart_type || 'line';
		if (type === 'pie') {
			type = 'percentage';
		}
		...
		if (data.columns) {
			...
			x = columns.filter(function (col) {
				return col[0] === data.x;
			})[0];

			if (x && x.length) {
				var dataset_length = x.length;
				var dirty = false;
...
frappe.chart.utils = {};
frappe.chart.utils.float_2 = function (d) {
...
$$.runSVGAnimation = function (svg_container, elements) {
	var parent = elements[0][0]['unit'].parentNode;
	...
	elements.map(function (element) {
		...
		var _$$$animateSVG = $$.animateSVG.apply($$, _toConsumableArray(element));

		var _$$$animateSVG2 = _slicedToArray(_$$$animateSVG, 2);
...
$$.animateSVG = function (element, props, dur) {
	...
	var anim_element = element.cloneNode(false);
	...
	for (var attributeName in props) {
		...
		var current_value = element.getAttribute(attributeName);
